...
namespace cxy {
    namespace cxy_lmicp_lib {
        ...
        bool cxy_icp_arti_ik<_Scalar, _MinimizerType>::setKinematicChain(std::shared_ptr<cxy_icp_kinematic_chain<_Scalar>> kc)
        {
    ...
    template<typename _Scalar, int _MinimizerType>
        _Scalar cxy_icp_arti_ik<_Scalar, _MinimizerType>::icp_run(Eigen::Matrix< _Scalar, Eigen::Dynamic, 1> &x)
        {
    ...
    template<typename _Scalar, int _MinimizerType>
        cxy_icp_arti_ik<_Scalar, _MinimizerType>::cxy_icp_arti_ik() :  hasSetKC_(false)
        ...
        template<typename _Scalar, int _MinimizerType>
        cxy_icp_arti_ik<_Scalar, _MinimizerType>::~cxy_icp_arti_ik()
        {
            
        }

    template<typename _Scalar, int _MinimizerType>
        int cxy_icp_arti_ik<_Scalar, _MinimizerType>::icp_prepare_cost_function()
        {
    ...
    template<typename _Scalar, int _MinimizerType>
        _Scalar cxy_icp_arti_ik<_Scalar, _MinimizerType>::icp_minimization(Eigen::Matrix< _Scalar, Eigen::Dynamic, 1> &x)
        {
            ...
            if ( cxy_optimization::cxy_nonlinear_method::EIGEN_MINPACK == state)
            {
                ...
                for (int ii = 0; ii < kc_->size(); ii++)
                {
                    func_ = std::make_shared<cxy_lmicp_lib::cxy_icp_arti_ik_func<_Scalar>>(1, kc_, this->dataCloud_, this->kdtreeptr_, ii, x);
